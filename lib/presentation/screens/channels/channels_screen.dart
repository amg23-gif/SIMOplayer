import 'package:flutter/material.dart';
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    icon: Icon(vm == _VM.grid ? Icons.view_list_rounded : Icons.grid_view_rounded,
                      color: const Color(0xFF8B949E)),
                    onPressed: () => ref.read(_vmProv.notifier).state = vm == _VM.grid ? _VM.list : _VM.grid),
                ],
              ),
            ),
            Expanded(
              child: chAsync.when(
                ...
                error: (_, __) => const Center(
                  child: Text('خطأ في التحميل', style: TextStyle(fontFamily: 'Cairo', color: Color(0xFF8B949E)))),
                data: (channels) {
                  final list = q.isEmpty
                      ? channels
                      : channels.where((c) => c.name.toLowerCase().contains(q.toLowerCase())).toList();
                  if (list.isEmpty) return Center(
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      const Icon(Icons.search_off_rounded, color: Color(0xFF30363D), size: 50),
                      const SizedBox(height: 12),
                      Text('لا نتائج لـ "$q"', style: const TextStyle(fontFamily: 'Cairo', color: Color(0xFF8B949E))),
                    ]));
                  return vm == _VM.grid ? _GridV(list) : _ListV(list);
                },
              ),
            ),
          ],
  ...
  class _Tile extends StatelessWidget {
    final Channel ch; final bool isGrid;
    const _Tile({required this.ch, required this.isGrid});

    void _open(BuildContext context) => context.go(AppConstants.routePlayer, extra: {
      'channelId': ch.id,
    ...
    Widget build(BuildContext context) {
      final logo = ch.logoUrl != null && ch.logoUrl!.isNotEmpty
          ? CachedNetworkImage(imageUrl: ch.logoUrl!, fit: BoxFit.contain,
              errorWidget: (_, __, ___) => const Icon(Icons.live_tv, color: Color(0xFF30363D), size: 24))
          : const Icon(Icons.live_tv, color: Color(0xFF30363D), size: 24);
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Expanded(child: Padding(padding: const EdgeInsets.all(8), child: logo)),
                Container(
      ...
      return ListTile(
        ...
        leading: Container(
          ...
          child: ClipRRect(borderRadius: BorderRadius.circular(6), child: logo),
        ),
        title: Text(ch.name, style: const TextStyle(fontFamily: 'Cairo', color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
        subtitle: ch.category.isNotEmpty
            ? Text(ch.category, style: const TextStyle(fontFamily: 'Cairo', color: Color(0xFF8B949E), fontSize: 11))
            : null,
